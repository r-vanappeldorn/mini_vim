...
Editor::Editor(mini_vim::domain::events::EventManager& e) : eventManager(e) {
  ...
  this->modeMap[mode::Mode::Command] = std::make_unique<Command>();
}

void Editor::process(EditorState& editorState) {
  char key = editorState.getKeyPressed();

  if (key == (int)(unsigned) Keys::Esc) {
    editorState.setMode(mode::Mode::Normal);
    eventManager.emit(events::Event::MODE_CHANED, 10);
    return;
  }

  if (editorState.getMode() == mode::Mode::Normal && editorState.getKeyPressed() == (int)(unsigned char) Keys::Colon)  {
    editorState.setMode(mode::Mode::Command);
    eventManager.emit(events::Event::MODE_CHANED, 10);
    return;
  }
